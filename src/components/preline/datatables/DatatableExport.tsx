...
function DatatableExport({ pdfContent }: DatatableExportProps) {
  const handleExportPdf = () => {
    pdfMake.vfs = pdfFonts.pdfMake.vfs;
    pdfMake.createPdf(pdfContent).download("example.pdf");
  };
  return (
    <div
    ...
    >
      <button
        ...
        aria-label="Dropdown"
        onClick={() => {
          if ($(`#exportOptions`).css("opacity") === "0") {
            $(`#exportOptions`).css({
              opacity: 100,
              display: "block",
            });
      ...
      <div
        id={`exportOptions`}
        className="absolute top-full -left-0 hs-dropdown-menu hs-dropdown-open:opacity-100 w-32 transition-[opacity,margin] duration opacity-0 hidden z-20 bg-white rounded-xl shadow-[0_10px_40px_10px_rgba(0,0,0,0.08)] dark:shadow-[0_10px_40px_10px_rgba(0,0,0,0.2)] dark:bg-neutral-900"
        role="menu"
      ...
      >
        <div className="p-1 space-y-0.5">
          <button
            type="button"
            className="flex w-full items-center gap-x-2 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700"
